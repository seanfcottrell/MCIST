...
#NMI_Mclust_GraphST = normalized_mutual_info_score(obs_df['MCSER_spatial_domains'],  obs_df['ground_truth'])
...
#NMI_Leiden_GraphST = normalized_mutual_info_score(obs_df['MCSER_spatial_domains'],  obs_df['ground_truth'])
...
# Mclust
#adata4 = adata.copy()
#adata4 = MCSER_GATE(adata = adata4, n_clusters = n, spatial_rad_cutoff = 75, clustering_algo='Mclust')

#obs_df = adata4.obs.dropna()
#NMI_Mclust_GATE = normalized_mutual_info_score(obs_df['MCSER_spatial_domains'],  obs_df['ground_truth'])
#print('MCIST Mclust GATE NMI = %.5f' %NMI_Mclust_GATE)

# Leiden
#adata5 = adata.copy()
#adata5 = MCSER_GATE(adata = adata3, n_clusters = n, spatial_rad_cutoff = 75, clustering_algo='Leiden')

#obs_df = adata5.obs.dropna()
#NMI_Leiden_GATE = normalized_mutual_info_score(obs_df['MCSER_spatial_domains'],  obs_df['ground_truth'])
...
NMI_Mclust_SpaceFlow = normalized_mutual_info_score(obs_df['MCSER_spatial_domains'],  obs_df['ground_truth'])
...
NMI_Leiden_SpaceFlow = normalized_mutual_info_score(obs_df['MCSER_spatial_domains'],  obs_df['ground_truth'])
print('MCIST Leiden SpaceFlow NMI = %.5f' %NMI_Leiden_SpaceFlow)
####################################################################################################

#if section_id == '19':
 #       plt.rcParams["figure.figsize"] = (8, 8)
  #      sc.pl.spatial(adata2, color='MCSER_spatial_domains', cmap = 'tab10', save='merfish'+section_id+'.png', spot_size=20)
